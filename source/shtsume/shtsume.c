                                 ...
                                 tbase_t         *tbase )
{
    ...
    while(list->tdata.pn){
        ...
        if(list->next) thdata.pn = (list->next)->tdata.pn+1;
    ...
    while(mvlist->tdata.sh < list->tdata.sh && !list->inc)
    {
        ...
        tmp = unknown_mvlist(list);
        if(!tmp){
            g_error = true;
            sprintf(g_error_location, "%s line %d", __FILE__, __LINE__);
            shtsume_error_log(sdata, list, tbase);
            mvlist_free(list);
            return;
        }
        while(tmp->tdata.pn){
            if(tmp->tdata.pn >= INFINATE-1){
                SDATA_PRINTF(sdata,PR_BOARD);
                printf("info string error occured at %s line %d",
                       __FILE__,__LINE__);
                exit(EXIT_FAILURE);
            }
            if(tmp->next) thdata.pn = (tmp->next)->tdata.pn+1;
            else thdata.pn = INFINATE-1;
            thdata.sh = TSUME_MAX_DEPTH-sdata->core.count-1;
            memcpy(&sbuf, sdata, sizeof(sdata_t));
            sdata_move_forward(&sbuf, tmp->mlist->move);
